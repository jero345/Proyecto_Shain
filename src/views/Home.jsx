...
export const Home = () => {
  ...
  // 1) Hidratar desde cache inmediatamente (sin parpadeo)
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [summaryData, movements] = await Promise.all([
          ...
          getLastMovements(daysRange),
        ]);

        if (summaryData) {
          setSummary(summaryData);
          writeCache(SUMMARY_CACHE_KEY(todayDate), summaryData);
        }
        if (movements?.length) {
        ...
        if (!summary) setError("No se pudo cargar el resumen financiero");
      } finally {
        setLoading(false);
      }
    };
